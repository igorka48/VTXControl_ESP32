...
VTXControl::VTXControl(int vtxMode, int softPin, int responseTimeOut /*= 1000*/,
                       bool smartBaudRate /*= true*/, int numtries /*= 3*/) {
  DEBUG("VTXControl: Create");
  vtx_mode = vtxMode;
  // port = new SoftwareSerialWithHalfDuplex(softPin, softPin, false, false);
  port = new SoftwareSerialAdapter(softPin, softPin, false);
...
bool VTXControl::sa_readResponse() {
...
#endif
  // special feature to correctly read Eachine TX5258 response, it generates
  // first 0x00, 0x00 byte before sync_byte in response so we skip zero bytes at
  // start of packet
  uint8_t b = port->peek();
  int skippedbytes = 0;
  while (b == 0x00) {
  ...
  if (incoming_bytes_count < response_header_size) {
    DEBUG("sa readResponse, incoming_bytes_count < response_header_size: " +
          (String)incoming_bytes_count + ", return false");
    setError(VTXErrors::vtxIncomingBytesLessHeaderSize);
    return false;
  }
  DEBUG("sa readResponse, incoming_bytes_count = " +
        (String)incoming_bytes_count);
  ...
  if (b != SMARTAUDIO_SYNC_BYTE) {
    DEBUG("readResponse, b != SMARTAUDIO_SYNC_BYTE (b=" + (String)b +
          "), return false");
  ...
  // implementations that ignore the CRC also appear to not account for it in
  // the frame length
  // if (sa_ignoreCrc())
  //{
  //  header->length++;
  ...
  if (packet_size >= AP_SMARTAUDIO_MAX_PACKET_SIZE) {
    DEBUG("readResponse, rest packet size >= AP_SMARTAUDIO_MAX_PACKET_SIZE, "
          "return false");
  ...
  if (buffer_length < packet_size + response_header_size) {
    DEBUG("readResponse, buffer_length < packet_size + response_header_size, "
          "return false");
...
bool VTXControl::sa_parseResponseBuffer(const uint8_t *buffer) {
  ...
  const uint8_t headerPayloadLength =
      fullFrameLength - 1;              // subtract crc byte from length
...
bool VTXControl::trampReadResponse() {
  ...
  while (port->available() == 0) {
    if (currentTime > _responseTimeOut) {
      DEBUG("trampReadResponse, TIMEOUT");
      return false;
    }
    waitForInMs(100);
    currentTime += 100;
  ...
  // now we have no-zero length response, we can dump it
#if VTXCDEBUG
  DEBUG("trampReadResponse: Dump Received from port:");
  port->dumpReceiveBuffer();
#endif
  Serial.printf(
      "trampReadResponse: incoming_bytes_count: %d. TRAMP_FRAME_LENGTH: %d\n",
      incoming_bytes_count, TRAMP_FRAME_LENGTH);
  if (incoming_bytes_count == TRAMP_FRAME_LENGTH) {
...
#if VTXCDEBUG
    DEBUG("trampReadResponse: Dump VTXControl receive buffer:");
    ...
    // clear port
    port->flush();
    // successful response, wait another 100ms to give the VTX a chance to
    // recover before sending another command.
    delay(100); // ����� ����� ������������ delay
